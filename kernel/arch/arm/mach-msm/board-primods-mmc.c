/* linux/arch/arm/mach-msm/board-primods-mmc.c
...
static unsigned int primods_sd_slot_type = MMC_TYPE_SD;
static struct mmc_platform_data sdc1_plat_data = {
	.ocr_mask	= MMC_VDD_28_29,
	.translate_vdd  = msm_sdcc_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.slot_type	= &primods_sd_slot_type,
	.msmsdcc_fmin	= 144000,
...
static void __init msm7x27a_init_mmc(void)
{
	...
	/* SDIO WLAN slot */
    /* SDCC : WLAN chip is connected; we do in primods_init_mmc*/
#ifdef CONFIG_MMC_MSM_SDC2_SUPPORT
 ...
 */
static struct embedded_sdio_data primods_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
primods_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int primods_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int primods_wifi_status(struct device *dev)
{
	return primods_wifi_cd;
}

static unsigned int primods_wifislot_type = MMC_TYPE_SDIO_WIFI;
static struct mmc_platform_data primods_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = primods_wifi_status,
	.register_status_notify = primods_wifi_status_register,
	.embedded_sdio          = &primods_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.slot_type		= &primods_wifislot_type,
	.msmsdcc_fmin   = 400000,
...
int primods_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(primods_wifi_set_carddetect);

int primods_wifi_power(int on)
{
	...
	gpio_set_value(PRIMODS_GPIO_WIFI_SHUTDOWN_N, on); /* WIFI_SHUTDOWN */

	mdelay(120);
	return 0;
}
EXPORT_SYMBOL(primods_wifi_power);

int primods_wifi_reset(int on)
{
...
int __init primods_init_mmc(unsigned int sys_rev)
{
	uint32_t id;
	printk(KERN_INFO "%s\n", __func__);
	...
	id = GPIO_CFG(PRIMODS_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
	gpio_tlmm_config(id, 0);
	gpio_set_value(PRIMODS_GPIO_WIFI_SHUTDOWN_N, 0);

	/* wifi is SDCC2*/
	msm_add_sdcc(2, &primods_wifi_data);
	return 0;
}
