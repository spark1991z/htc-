...
#if 0
    if(wifi_already_on == 1)
    {
	   printk("%s: wifi already on -> turn off power\n", __func__);
	   unplug_device(10);
	   wifi_already_on = 0;
	}
...
static int emapi_disconnect_bus(void)
{
	...
	printk("emapi_disconnect_bus leave\n");
...
static int ti_sdio_rw_direct(struct sdio_request *req, int rw)
{
    ...
    if (rw)
    {
		if(copy_from_user(__emapi_data_buffer, req->buf, req->len)) {
			...
			iStatus = sdioDrv_WriteSyncBytes (SDIO_WIFI_FUNC_NUM, req->addr, __emapi_data_buffer, req->len, 0);
		}
    }
    else
    {
        iStatus = sdioDrv_ReadSyncBytes (SDIO_WIFI_FUNC_NUM, req->addr, __emapi_data_buffer, req->len, 0);
		CHECK_ERR(iStatus);
	...
	if (rw) {
		if(copy_from_user(__emapi_data_buffer, req->buf, len)) {
			...
			while (req->len > SDIO_BLK_SIZE) {
                ...
                sdioDrv_WriteSync(SDIO_WIFI_FUNC_NUM, req->addr, ptr, SDIO_BLK_SIZE,1, 0);
				req->len -= SDIO_BLK_SIZE;
				ptr += SDIO_BLK_SIZE;
				req->addr+=SDIO_BLK_SIZE;
			}
			sdioDrv_WriteSync(SDIO_WIFI_FUNC_NUM, req->addr, ptr, req->len,1, 0);
		}
	} else {
		while (req->len > SDIO_BLK_SIZE) {
			//ptr_ti_sdio_drv_data->sdio_read(SDIO_WIFI_FUNC_NUM, req->addr, ptr, SDIO_BLK_SIZE, 0, 1, 0);
            sdioDrv_ReadSync(SDIO_WIFI_FUNC_NUM, req->addr, ptr, SDIO_BLK_SIZE, 1, 0);
			req->len -= SDIO_BLK_SIZE;
			ptr += SDIO_BLK_SIZE;
			req->addr+=SDIO_BLK_SIZE;
		}
		sdioDrv_ReadSync(SDIO_WIFI_FUNC_NUM, req->addr, ptr, req->len,1, 0);
		CHECK_ERR(iStatus);
