...
// static int msm_rmnet_debug_mask = 7;
...
static int __rmnet_open(struct net_device *dev)
{
	...
	struct rmnet_private *p = netdev_priv(dev);
	// SSD_RIL
	// pr_info(MODULE_NAME "[%s] __rmnet_open()\n", dev->name);
	...
	// SSD_RIL
	// pr_info(MODULE_NAME "[%s] __rmnet_open() msm_rmnet_load_modem done\n", dev->name);
	if (!p->ch) {
	...
	smd_disable_read_intr(p->ch);
	// SSD_RIL
	// pr_info(MODULE_NAME "[%s] __rmnet_open() success\n", dev->name);
	return 0;
}
...
static int rmnet_open(struct net_device *dev)
{
	...
	// SSD_RIL
	// pr_info(MODULE_NAME "[%s] rmnet_open() return: %d\n", dev->name, rc);
	return rc;
}
